...
int main() {
	...
	// GL states, we'll enable depth testing and backface culling
	...
	if (loadScene) {
		...
		MeshResource::Sptr gridMesh = ResourceManager::CreateAsset<MeshResource>("grid.obj");
		MeshResource::Sptr snakeMesh = ResourceManager::CreateAsset<MeshResource>("snakepart.obj");
		Texture2D::Sptr    gridTexture = ResourceManager::CreateAsset<Texture2D>("textures/snakegrid.png");
		Texture2D::Sptr    snakeTexture = ResourceManager::CreateAsset<Texture2D>("textures/snakecolor.png");
		...
		Material::Sptr gridMaterial = ResourceManager::CreateAsset<Material>();
		{
			gridMaterial->Name = "Grid";
			gridMaterial->MatShader = scene->BaseShader;
			gridMaterial->Texture = gridTexture;
			gridMaterial->Shininess = 2.0f;
		}

		Material::Sptr snakeMaterial = ResourceManager::CreateAsset<Material>();
		{
			snakeMaterial->Name = "Snake";
			snakeMaterial->MatShader = scene->BaseShader;
			snakeMaterial->Texture = snakeTexture;
			snakeMaterial->Shininess = 2.0f;
		}
		...
		GameObject::Sptr grid = scene->CreateGameObject("Grid");
		{
			...
			grid->SetPostion(glm::vec3(0.0f, 0.0f, 0.0f));
			grid->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			//create and attach renderer for grid
			RenderComponent::Sptr renderer = grid->Add<RenderComponent>();
			renderer->SetMesh(gridMesh);
			renderer->SetMaterial(gridMaterial);

			//Add static rigid body
			RigidBody::Sptr physics = grid->Add<RigidBody>(RigidBodyType::Kinematic);
